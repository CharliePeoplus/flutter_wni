#import "FlutterWNInterfacePlugin.h"
#import "KeychainItemWrapper.h"

@implementation FlutterWNInterfacePlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  FlutterMethodChannel* channel = [FlutterMethodChannel
      methodChannelWithName:@"flutter_wni_plugin"
            binaryMessenger:[registrar messenger]];
  FlutterWNInterfacePlugin* instance = [[FlutterWNInterfacePlugin alloc] init];
...
- (NSString *)deviceUUID {
    ...
    NSString *appIdentifier = [bundleIdentifier stringByAppendingString:@".uuid"];

    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *cachedUUID = [userDefaults stringForKey:appIdentifier];

    if (cachedUUID != nil) {
        return cachedUUID;
    }

    KeychainItemWrapper *wrapper = [[KeychainItemWrapper alloc] initWithIdentifier:appIdentifier accessGroup:nil];
    ...
    [userDefaults setObject:uuid forKey:appIdentifier];
...
- (void)constantsToExport:(void  (^ _Nullable)(NSDictionary * _Nonnull constants))completionHandler
{
    ...
    NSString *appName = [mainBundle objectForInfoDictionaryKey:@"CFBundleName"] ?: [NSNull null];
    NSString *appVersion = [mainBundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"] ?: [NSNull null];
    NSString *buildVersion = [mainBundle objectForInfoDictionaryKey:@"CFBundleVersion"] ?: [NSNull null];
    ...
    [self getWebViewUserAgent:^(NSString * _Nullable webViewUserAgent, NSError * _Nullable error) {
        completionHandler(@{
            @"interfaceVersion": @"v1",
            @"appId": mainBundle.bundleIdentifier ?: @"",
            @"appName": appName,
            ...
            @"deviceModel": [[currentDevice model] lowercaseString],
            ...
            @"packageUserAgent": [NSString stringWithFormat:@"%@/%@.%@ %@", appName, appVersion, buildVersion, userAgent],
            @"userAgent": userAgent,
